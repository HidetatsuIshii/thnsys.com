...
let activeBranchFilter = 'oji'; // ★追加: 候補者リストの初期タブ状態
   ...
   if (json.status === 'success') {
      ...
      if (json.data && json.data.users) {
          json.data.users.forEach(u => {
              // 所属(branch)が空ではなく、かつ「王子」ではない場合
              if (u.branch && u.branch !== 'oji') {
    ...
    columns.forEach(col => {
        ...
        reservations.forEach(res => {
            ...
            if (sHour < END_HOUR && (sHour > START_HOUR || (sHour === START_HOUR && sMin >= 0))) {
                ...
                bar.onclick = (e) => {
                    if (typeof isTouch !== 'undefined' && !isTouch && typeof hasDragged !== 'undefined' && hasDragged) return;
                    e.stopPropagation();
                    openDetailModal(res);
                };
...
function openModal(res = null, defaultRoomId = null, clickHour = null, clickMin = 0, clickDateStr = null) {
  ...
  if(document.getElementById('check-tentative')) {
    ...
    if (leftTabsContainer) {
        ...
        if(branchTabs.length > 0) {
            ...
            branchTabs[0].classList.add('active'); // 常に「王子」を初期状態に
        }
     }
  // 編集用・新規用のエリア表示リセット
  ...
  if (res) {
    ...
    if (res.customerCard) {
        const radio = document.querySelector(`input[name="input-customer-card"][value="${res.customerCard}"]`);
...
async function saveBooking() {
    ...
    const cardRadio = document.querySelector('input[name="input-customer-card"]:checked');
    const customerCardVal = cardRadio ? cardRadio.value : "";
    ...
    for (const resData of reservationList) {
        const params = {
            ...
            title: title,
            note: note,
            customerCard: customerCardVal, // ★追加
...
function renderGuestShuttle() {
    ...
    masterData.users.forEach(u => {
        if (u.branch && u.branch !== 'oji') return;
